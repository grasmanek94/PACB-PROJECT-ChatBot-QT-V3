...
namespace ProxyScraper
{
    public class Scraper
    {
        ...
        public Scraper()
        {
            ...
            {
                string url = "http://www.vipsocks24.net/search?updated-max=" + date.Year.ToString() + "-" + date.Month.ToString("D2") + "-" + date.Day.ToString("D2") + "T21:06:00%2B06:00&max-results=2";
                var Webget = new HtmlWeb();
                var doc = Webget.Load(url);
                foreach (HtmlNode link in doc.DocumentNode.SelectNodes("//a[@href]"))
                {
                    string href_url = link.Attributes["href"].Value;
                    if (href_url.StartsWith("http://www.vipsocks24.net/" + date.Year.ToString() + "/" + date.Month.ToString("D2") + "/" + date.Day.ToString("D2") + "-" + date.Month.ToString("D2") + "-" + (date.Year - 2000).ToString("D2")))
                    {
